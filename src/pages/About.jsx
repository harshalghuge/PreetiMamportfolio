...
export const About = () => {
  ...
  const journeyNarrative = [
    ...
    {
      title: "The Searching",
      // FIX: renamed from TheSearching.jpg → thesearching.jpg (use lowercase on server)
      image: "/images/IMG_99631.jpg",
      paragraphs: [
    ...
    },
    {
      ...
      // FIX: renamed from MissingEducation.avif → missingeducation.avif (use lowercase on server)
  ...
  return (
    <div className="bg-[#FAF9F6] text-stone-900 overflow-x-hidden">
      ...
      {/* FIX: Removed per-paragraph motion animations (was 28 animated elements).
               Now only the image and heading animate — much lighter on the browser. */}
          ...
          >
            <div className="max-w-7xl mx-auto px-6">
              <motion.img
                ...
                viewport={{ once: true }}
                // FIX: Reduced scale animation from 1.2→1 to 1.05→1 (less GPU work)
                // FIX: Reduced duration from 1.5s to 0.8s
                transition={{ duration: 0.8 }}
                src={section.image}
                // FIX: Added loading="lazy" — images only load when user scrolls to them
                loading="lazy"
                alt={section.title}
              ...
              {/* FIX: Replaced individual <motion.p> per paragraph with a single animated wrapper */}
              <motion.div
      ...
      {/* --- CTA --- */}
      {/* FIX: Replaced external Unsplash URL with a local image.
               Download that image and save it as /images/cta-bg.jpg in your project.
               External URLs load slowly and depend on Unsplash's CDN being fast. */}
